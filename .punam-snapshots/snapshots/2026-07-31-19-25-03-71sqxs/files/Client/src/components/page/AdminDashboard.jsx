        ...
        <section className={styles.Section}>
          ...
          {editingEvent && (
            <form className={styles.FormCard} onSubmit={(e) => {
              ...
              const payload = {
                ...eventForm,
                id: editingEvent === 'new' ? `admin-ev-${Date.now()}` : (editingEvent.id || editingEvent.event_id),
                event_id: editingEvent === 'new' ? `admin-ev-${Date.now()}` : (editingEvent.id || editingEvent.event_id),
                title: eventForm.title,
              };
              const prev = readAdminEvents();
              let next;
              if (editingEvent === 'new') next = [payload, ...prev];
              else next = prev.map((x) => String(x.id || x.event_id) === String(payload.id) ? { ...x, ...payload } : x);
              if (editingEvent !== 'new' && !prev.find((x) => String(x.id || x.event_id) === String(payload.id))) {
                next = [payload, ...prev];
              }
              ...
              pushToast?.('Event saved — visible on Events page', 'success');
