...
import { MasterSearch, PlaceCard } from '@components';
...
export function HomePage() {
  ...
  const { places, loading: placesLoading } = usePlaces({ category: 'all', sort: 'rating' });
  ...
  return (
    <main className={styles.Page}>
      ...
      <section className={styles.Section} aria-labelledby="top-places">
        <div className={styles.SectionHead}>
          <h2 id="top-places">Top-rated spots</h2>
        ...
        <div className={styles.PlaceRail}>
          ...
          {!placesLoading &&
            topPlaces.map((place) => (
              <div key={place.place_id ?? place.id} className={styles.PlaceRailItem}>
                <PlaceCard place={place} />
              </div>
            ))}
        </div>
        <div className={styles.QuickCats}>
          <Link to="/explore?category=eats" className={styles.QuickCat}>
            <Coffee size={15} /> Best eats
          </Link>
          <Link to="/explore?category=nature" className={styles.QuickCat}>
            <Trees size={15} /> Nature escapes
          </Link>
          <Link to="/explore?category=nightlife" className={styles.QuickCat}>
            <Music2 size={15} /> Nightlife
          </Link>
          <Link to="/explore?budget=under1500" className={styles.QuickCat}>
            <Wallet size={15} /> Under 1,500
          </Link>
        </div>
      </section>
