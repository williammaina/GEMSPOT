...
export function ExplorePage() {
  ...
  /** Geocoded map focus from the search box (e.g. "Kilimani") */
  const [mapFocus, setMapFocus] = useState(null);
  const [geocodeStatus, setGeocodeStatus] = useState(''); // '', 'loading', 'ok', 'miss'
  ...
  // When the user searches an area name, resolve it to lat/lng and fly the map there
  useEffect(() => {
    ...
    // Prefer a matching place's coordinates when the search hits curated data
  ...
  return (
    <main className={styles.LayoutSplit}>
      ...
      <section className={styles.RightColumn} aria-label="Map and insights">
          ...
          <div className={styles.MapHead}>
            <h2 className={styles.MapTitle}>
              ...
              {mapFocus?.label
                ? mapFocus.label
                : urlFilters.query
                  ? `Map · ${urlFilters.query}`
                  : 'Map of results'}
            </h2>
            {geocodeStatus === 'loading' && (
              <span className={styles.MapStatus}>Finding location…</span>
            )}
            {geocodeStatus === 'ok' && mapFocus && (
              <span className={styles.MapStatusOk}>Pinned on map</span>
            )}
            {geocodeStatus === 'miss' && urlFilters.query && (
              <span className={styles.MapStatusMiss}>No exact pin — showing places</span>
            )}
          </div>
