    ...
    <main className={styles.PageContainer}>
      ...
      <div className={styles.ContentSplit}>
        <section className={styles.MainColumn} aria-labelledby="place-title">
          ...
          {(place.category === 'eats' || place.category === 'cafe') &&
            Array.isArray(place.menuHighlights) &&
            place.menuHighlights.length > 0 && (
              <section className={styles.SectionBlock} aria-label="Menu highlights">
          ...
          {place.category === 'nature' &&
            ...
            place.activities.length > 0 && (
              <section className={styles.SectionBlock} aria-label="Activities">
                <h2 className={styles.SectionHeading}>What you can do</h2>
                <ul className={styles.ActivityList}>
                  {place.activities.map((act, i) => (
                    <li key={i} className={styles.ActivityItem}>
                      <strong>{typeof act === 'string' ? act : act.name}</strong>
                      <span>
                        {[act?.duration, act?.intensity].filter(Boolean).join(' · ')}
                      </span>
                    </li>
                  ))}
                </ul>
                {(place.requirements || place.whatToBring) && (
                  <p className={styles.GearNote}>
                    Bring: {Array.isArray(place.whatToBring) ? place.whatToBring.join(', ') : place.whatToBring || place.requirements}
                  </p>
                )}
              </section>
            )}
