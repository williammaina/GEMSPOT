...
export function normalizeEvent(raw) {
  ...
  if (hostRaw && typeof hostRaw === 'object') {
    hostName = [hostRaw.name, hostRaw.org].filter(Boolean).join(' · ');
  } else if (hostRaw) {
    hostName = String(hostRaw);
  } else {
  ...
  return {
    ...
    // Always a display string so React never renders {name, org}
    host: hostName || undefined,
    hostLabel: hostName || raw.venue_name || 'GemSpot host',
    host_name: raw.host_name || (typeof hostRaw === 'object' ? hostRaw?.name : hostName),
  };
}
