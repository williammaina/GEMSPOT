...
import { LocationCard } from '../shared/LocationCard.jsx';
...
export function ExplorePage() {
  ...
  return (
    <main className={styles.LayoutSplit}>
      ...
      <aside className={styles.MapSection} aria-label="Locations">
        <div className={styles.LocationHub}>
          <p className={styles.HubEyebrow}>Nearby &amp; featured</p>
          <h2 className={styles.HubTitle}>Locations</h2>
          <p className={styles.HubText}>
            Tap a spot for directions, matatu tips, and a map — no Mapbox token needed.
          </p>
          <ul className={styles.HubList}>
            {(places || []).slice(0, 8).map((place) => {
              ...
              return (
                <li key={id}>
                  <Link to={`/place/${id}`} className={styles.HubItem}>
                    <span className={styles.HubItemTitle}>{place.title || place.name}</span>
                    <span className={styles.HubItemMeta}>{place.location || place.town || 'Kenya'}</span>
                  </Link>
                </li>
              );
            })}
            {!loading && (!places || places.length === 0) && (
              <li className={styles.HubEmpty}>No places in this filter.</li>
            )}
          </ul>
          {(places || [])[0] && (
            <div className={styles.HubMap}>
              <LocationCard
                compact
                name={places[0].title || places[0].name}
                address={places[0].address}
                town={places[0].town}
                county={places[0].county}
                matatu={places[0].matatu || places[0].matatu_route}
