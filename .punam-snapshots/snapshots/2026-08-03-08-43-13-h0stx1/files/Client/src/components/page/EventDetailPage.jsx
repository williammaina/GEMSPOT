...
export function EventDetailPage() {
  ...
  useEffect(() => {
    ...
    const q = [event.location, event.venue_name, 'Kenya'].filter(Boolean).join(', ');
    if (!q) return undefined;
    let cancelled = false;
    if (
      Number.isFinite(Number(event.lat ?? event.latitude)) &&
      Number.isFinite(Number(event.lng ?? event.longitude))
    ) {
      const lat = Number(event.lat ?? event.latitude);
      const lng = Number(event.lng ?? event.longitude);
      setMapsEmbed(`https://maps.google.com/maps?q=${lat},${lng}&z=15&output=embed`);
      return undefined;
    }
    // text query embed while geocoding
    setMapsEmbed(
      `https://maps.google.com/maps?q=${encodeURIComponent(q)}&z=14&output=embed`
    );
    geocodeLocation(q).then((hit) => {
      if (cancelled || !hit) return;
      setMapsEmbed(`https://maps.google.com/maps?q=${hit.lat},${hit.lng}&z=15&output=embed`);
    });
    return () => {
      cancelled = true;
    };
        ...
        <div className={styles.MainCol}>
          ...
          <section className={styles.Section}>
            ...
            <a
              ...
              href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([event.location, event.venue_name, 'Kenya'].filter(Boolean).join(', '))}`}
              target="_blank"
              rel="noreferrer"
            >
