...
/**
 * Dedicated map section (Explore, etc). Pass `focus` from geocodeLocation().
 * Not related to Today's pick.
 */
export function MapboxCanvas({ places = [], focus = null, focusLabel = '' }) {
  const center = useMemo(() => {
    ...
    if (first) {
      return [Number(first.longitude ?? first.lng), Number(first.latitude ?? first.lat)];
    }
        ...
        <div className={styles.FallbackPanel}>
          <div className={styles.FallbackHeader}>
            ...
            <p className={styles.FallbackText}>
              Try “Kilimani”, “Westlands”, or “Karura”. Add{' '}
              <code>VITE_MAPBOX_ACCESS_TOKEN</code> for the full interactive map.
            </p>
          </div>
          <div className={styles.FallbackList} role="list">
            {(places || []).slice(0, 4).map((place) => (
              <div key={place.id || place.title} className={styles.FallbackItem} role="listitem">
