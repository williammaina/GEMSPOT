...
export function AppProvider({ children }) {
  ...
  // Wrap favorite toggle with toast when available
  const toggleFavorite = useCallback(
    async (placeId) => {
      const result = await favoritesApi.toggleFavorite?.(placeId);
      const on = favoritesApi.isFavorite?.(String(placeId));
      pushToast(on ? 'Saved to favorites' : 'Removed from favorites', 'success');
      return result;
    },
