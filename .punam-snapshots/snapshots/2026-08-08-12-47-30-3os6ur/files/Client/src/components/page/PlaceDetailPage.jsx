...
export function PlaceDetailPage() {
  ...
  const handleShare = async () => {
    if (!place) return;
    trackEvent('place_share', { id: place.place_id || place.id, category: place.category });
  ...
  const showPlanB =
    isOutdoorCategory(place.category) && isHighRainRisk(weather) && !weather.loading;
  const indoorAlts = showPlanB
    ? pickIndoorAlternatives(place, placesData, 4)
    : [];
  const weatherCondition = weather.loading
    ? 'Checking local weather…'
    : showPlanB
      ? planBMessage(weather)
        ...
        <aside className={styles.SideColumn} aria-label="Logistics and backup options">
          ...
          {showPlanB && (
            <PlanBAlert
              ...
              alternatives={indoorAlts}
              onAddToPlan={(p) => {
                addToPlan?.(p);
                trackEvent('plan_b_add', { from: place.place_id || place.id, to: p.place_id || p.id });
                pushToast?.(`Added ${p.title || p.name} as Plan B`, 'success');
              }}
            />
          )}
        </aside>
