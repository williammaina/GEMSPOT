  ...
  return (
    <section style={{ marginBottom: 24, padding: 16, borderRadius: 16, border: '1px solid var(--color-border)', background: 'var(--card-bg)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <button type="button" onClick={() => { clearAnalytics(); window.location.reload(); }} style={{ fontSize: 12, fontWeight: 700, cursor: 'pointer', background: 'transparent', border: '1px solid var(--color-border)', borderRadius: 8, padding: '4px 8px' }}>Clear</button>
      </div>
      <p style={{ margin: '0 0 12px', color: 'var(--color-text-muted)', fontSize: 14 }}>
        {summary.total} events · {summary.last24h} last 24h · guest blocks: {summary.guestToAuth?.detailBlocks || 0}
      </p>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div>
          <p style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase' }}>Top actions</p>
        ...
        <div>
          <p style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase' }}>Categories</p>
          <ul style={{ margin: 0, paddingLeft: 18, fontSize: 14 }}>
            {(summary.topCategories || []).length === 0 && <li>No data yet</li>}
            {(summary.topCategories || []).map((e) => (
              <li key={e.category}>{e.category}: <strong>{e.count}</strong></li>
            ))}
          </ul>
        </div>
      </div>
