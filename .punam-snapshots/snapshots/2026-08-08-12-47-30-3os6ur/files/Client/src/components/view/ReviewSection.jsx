...
import { rankReviews, isVerifiedLocal, reportMedia, isMediaReported } from '../../library/helpers/trustLayer.js';
...
export function ReviewSection({ reviewsData = [], placeId, onReviewAdded, onCrowdReport }) {
  ...
  const sortedReviews = useMemo(() => {
    ...
    // default / useful: keep rankReviews order
    return list;
  }, [merged, filter]);
            ...
            <div className={styles.ReviewContent}>
              <div className={styles.ReviewHeader}>
                <span className={styles.AuthorName}>
                  {review.author}
                  {isVerifiedLocal(review) && (
                    <span className={styles.LocalBadge} title="Verified local"> · Local</span>
                  )}
                </span>
              ...
              <button
                ...
                onClick={() => {
                  reportMedia(review.id, 'inappropriate');
                  pushToast?.('Media reported — thank you', 'info');
                }}
                disabled={isMediaReported(review.id)}
              >
                {isMediaReported(review.id) ? 'Reported' : 'Report photo/video'}
              </button>
            </div>
          </article>
        ))}
