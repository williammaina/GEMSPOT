...
def get_user_favorites():
    ...
    # Frontend often expects a flat list of place ids or place objects
...
def add_favorite(place_id):
    user_id = int(get_jwt_identity())
    return _add(user_id, place_id)


def _add(user_id, place_id):
    Place.query.get_or_404(place_id)
    existing = Favorite.query.filter_by(user_id=user_id, place_id=place_id).first()
    if existing:
        return jsonify({'message': 'Place is already favorited', 'place_id': place_id}), 200
    favorite = Favorite(user_id=user_id, place_id=place_id)
    db.session.add(favorite)
    db.session.commit()
    return jsonify({'message': 'Added to favorites', 'place_id': place_id}), 201
...
def remove_favorite(place_id):
    ...
    if not favorite:
        return jsonify({'message': 'Not in favorites'}), 200
